...
<h2>Score: ${data.overall_score}/100</h2>
<h2>Job Match Score: ${data.job_match_score ?? "N/A"}/100</h2>

<h3>Profile Summary</h3>
<p>${data.profile_summary || ""}</p>
<h3>Section Scores</h3>
<ul>
  <li>Education: ${data.section_scores?.education ?? "N/A"}/10</li>
  <li>Skills: ${data.section_scores?.skills ?? "N/A"}/10</li>
  <li>Experience: ${data.section_scores?.experience ?? "N/A"}/10</li>
  <li>Projects: ${data.section_scores?.projects ?? "N/A"}/10</li>
</ul>
<h3>Section Feedback</h3>
<ul>
  <li><b>Education:</b> ${data.section_feedback?.education || "N/A"}</li>
  <li><b>Skills:</b> ${data.section_feedback?.skills || "N/A"}</li>
  <li><b>Experience:</b> ${data.section_feedback?.experience || "N/A"}</li>
  <li><b>Projects:</b> ${data.section_feedback?.projects || "N/A"}</li>
</ul>


<h3>Key Strengths</h3>
<ul>
  ${(data.key_strengths || []).map(i => `<li>${i}</li>`).join("")}
</ul>

<h3>Areas for Improvement</h3>
<ul>
  ${(data.areas_for_improvement || []).map(i => `<li>${i}</li>`).join("")}
</ul>

<h3>Missing Skills</h3>
<ul>
  ${(data.missing_skills || []).map(i => `<li>${i}</li>`).join("")}
</ul>
...
<div>
  ${(data.keywords_found && data.keywords_found.length > 0)
    ? data.keywords_found.map(k => `<span class="kw-chip">${k}</span>`).join(" ")
...
document.getElementById("downloadBtn").addEventListener("click", async () => {
  ...
  if (!window.lastData) {
    alert("No data to download. Upload resume first.");
    return;
  }
...
// Function to describe the rating
function describeRating(rating) {
  if (rating >= 9) return "Excellent - Ready for top opportunities!";
  if (rating >= 7) return "Good - Minor tweaks needed";
  if (rating >= 5) return "Average - Needs improvement";
